...
(syntax-spec
  ...
  (extension-class term-macro)  ;; New
  (extension-class goal-macro)  ;; New

  (nonterminal term
    #:allow-extension term-macro  ;; New
  ...
  (nonterminal goal
    #:allow-extension goal-macro ;; New
...
(define-syntax run         ;; No binding rule, but hygienic.
  (syntax-rules ()
    [(run n (x0 x ...) g ...)
     (run n q (fresh (x0 x ...)
                (== (list x0 x ...) q) g ...))]
